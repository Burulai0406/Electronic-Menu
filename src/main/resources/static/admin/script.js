...
document.getElementById('loginBtn').addEventListener('click', ()=> {
    ...
    console.log("Нажата кнопка входа", u, p); // проверка
...
window.addEventListener('load', ()=>{
  if(authHeader){
    fetch(`${ADMIN_API}/categories`, { headers:{ Authorization: authHeader }})
      .then(r => r.ok ? setAuthHeader('admin','***') : clearAuth());
  }
});
  ...
  fetchAuth('/api/admin/categories')
    ...
    .then(data => {
      ...
      data.forEach(cat => {
        ...
        opt.value = cat.id;
        opt.text = `${cat.name_en || 'EN'} / ${cat.name_kg || 'KG'}`;
...
function addItem(){
  ...
  if(!categoryId) return showMessage('Выберите категорию');
...
function resetCategoryFields(){
  ['cat_name_kg','cat_name_en','cat_image'].forEach(id=>document.getElementById(id).value='');
}

function resetItemFields(){
  ['item_name_kg','item_name_en','item_desc_kg','item_desc_en','item_price','item_image']
      ...
      data.forEach(cat => {
        ...
        let html = `
          <div class="flex justify-between items-center">
            <div><strong>${cat.name_en || 'EN'} / ${cat.name_kg || 'KG'}</strong></div>
            <button onclick="deleteCategory(${cat.id})" class="px-2 py-1 rounded">Удалить</button>
          </div>
        `;
        ...
        if(cat.items?.length){
          ...
          cat.items.forEach(item => {
            html += `
              <div class="flex justify-between items-center">
                <div>${item.name_en || item.name_kg} (${item.price})</div>
                <button onclick="deleteItem(${item.id})" class="px-2 py-1 rounded">Удалить</button>
              </div>`;
          });
          html += '</div>';
        } else {
          html += '<div class="mt-2 text-sm text-gray-500">Пусто</div>';
        }
